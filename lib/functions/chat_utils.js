...
const updateChat = function (chat, options) {
    ...
    if (!chat.options.negotiating_dh && (options.diffie_hellman || String(options.key).match(/dh/i))) {
        ...
        var remote_port    = Number(chat.options.client_port) + 1 || 3356;
    ...
    if (options['export-key']) {
        if (chat.options.key) {
            var message = DateCypher.cypher(chat.options.key, gritoDoIpirangeDate).toString('base64');
            child_process.execFile(entrepixels, ['hide', '-m', message, '-i', gritoDoIpirangaImage,'-o', options['export-key']]);
        }
    }

    if (options['import-key']) {
        child_process.execFile(entrepixels, ['show', '-i', options['import-key']], function(error, stdout){
